...
<%  //웹브라우저가 게시글 목록을 캐싱할 경우 새로운 글이 추가되더라도 새글이 목록에 안 보일 수 있기 때문에 설정
	//캐시를 지워줘야 함 (그냥 한글 깨지는거 막는거처럼 항상 붙혀 넣어주는 느낌)
...
<a href="BoardList.jsp?page=1"><button>first</button></a>	
	<!-- 페이지번호 <a> 생성 -->
		<%for(int i=1;i<=totalPageCount;i++){ %>
			<a href='BoardList.jsp?page=<%=i%>'>[<%= i %>]</a>
		<%}//end of for %>
<a href="BoardList.jsp?page=<%=totalPageCount%>"><button>last</button></a>
