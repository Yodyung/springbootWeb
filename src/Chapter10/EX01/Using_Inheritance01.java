package Chapter10.EX01;

//클래스 내부 구성요소 : 1.필드, 2.생성자, 3.메소드, 4.이너 클래스

//클래스 외부 구성요소 : 1.패키지 2.import 3.외부 클래스

//상속(Inheritance) : 객체 지향 언어의 꽃
//		-부모(super class)클래스의 필드와 메소드, 이너 클래스를 자식 클래스에서 물려준다.
//		-자식(child class)클래스는 부모 클래스의 코드(
//		- 코드를 간결하게 사용. 유지 보수를 아주 쉽게 한다. 중복된 코드를 방지할 수 있다.
//다향성 작영: 부모 클래스와 자식 클래스에서 재정의해서 사용. (다양한 타입으로 사용이 가능하다.)

//부모 클래스: 자식 클래스의 공동적인 특징을 가지는 클래스

class Human {	//사람 : 부모클래스 : 자식 클래스의 공통적인 필드와 메소드를 선언
	String name;
	int age;
	
	//메소드 정의
	void eat(){
		System.out.println("모든 사람은 먹는다.");
	}
	
	void sleep() {
		System.out.println("모든 사람은 잠을 잔다.");
	}
	void print() {
		System.out.println(name+" , "+age);
	}
	
}
class Student extends Human {		//자식 클래스 : 부모 클래스의 필드와 메소드를 그대로 상속받는다.
						// ---------------자식 클래스 안의 특징이 적용된 필드와 메소드를 선언한다.------------
			int id;
	class student {			//human 클래스를 상속 받는다.
		
		//부모 클래스의 필드와 메소드가 그대로 내려온다.
		//자식 클래스의 특징을 가진 필드와 메소드를 선언.	
	}
	void goToschool() {		//자식 클래스에서 선언 가능한 메소드
		System.out.println("학생이 학교에 갑니다.");
	}
	
	
}
class worker extends Human {
			//부모클래스의 필드와 메소드가 그대로 내려온다.
		int id;
		
		void goTowork() {
			System.out.println("직장인이 일하러 갑니다.");
		}

}class Professor extends worker {
	int id;
	
	void goTocollage() {
		System.out.println("교수님이 일하러 갑니다.");
}

}

public class Using_Inheritance01 {

	public static void main(String[] args) {
		//1.human 클래스 객체 생성
		Human h=new Human();
		
		h.name="홍길동";
		h.age=30;
		h.eat();
		h.sleep();
		h.print();

		//2. Student 클래스의 객체 생성(자식)
		System.out.println("----------------------------student 객체 출력----------------------------");
		Student s=new Student();
		
		s.name="세종대왕";		//부모에서 상속된 필드 
		s.age=50;				//부모에서 상속된 필드 
		s.id=1000;				//클래스에 존재하는 필드 
		s.eat();
		s.sleep();
		s.print();
		s.goToschool();			//student 클래스의 메소드
		
		//3. worker 클래스의 객체 생성(자식)
		System.out.println("----------------------------worker 객체 출력----------------------------");
		worker w=new worker();
		
		w.name="예사랑";			//상속 필드
		w.age=29;				//상속 필드
		w.id=1001;				//자신의 필드
		
		w.eat();				//상속 메소드
		w.sleep();				//상속 메소드
		w.print();				//상속 메소드
		w.goTowork();			//자신의 메소드
		
		//4. Professor 클래스의 객체 생성(자식)
		System.out.println("----------------------------Professor 객체 출력----------------------------");
		Professor p=new Professor();
		
		p.name="예소은";			//상속 필드
		p.age=24;				//상속 필드
		p.id=1002;				//자신의 필드
		
		p.eat();				//상속 메소드
		p.sleep();				//상속 메소드
		p.print();				//상속 메소드
		p.goTocollage();		//자신의 메소드
	
		
	}

}
